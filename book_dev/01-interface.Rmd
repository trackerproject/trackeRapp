...
In order to get more detail on a particular session or sessions, the
user can select which sessions to display by clicking on the legend in
the top right corner of the plot, as displayed in Figure \@ref(fig:zonesIndividualSession), where only session 13 was
selected.

```{r zonesIndividualSession, out.width='95%', fig.align='center', fig.cap='Time in zones individual session'}
knitr::include_graphics('Images/zones_individual_session.png')
```
