...
sessions between 16:00-21:00. Hovering over session segments displays
...
The summary table (Figure \@ref(fig:summarySelectedSessions)) displays the date, the start time, the end time, the duration and the sport of each session, as well as it allows users to select sessions directly from the table. Currently selected sessions are highlighted with a blue colour. In order to accommodate for large datasets, the table is
...
sessions in a blue colour and all the unselected sessions are displayed in a grey colour colour. Hovering over a route displays a basic summary of the session. The basic summary displays the session number, 
...
their training pattern or detect changes in the intensity of their
performance.
...
with a blue colour and hovering over an individual point displays the
...
for the selected sessions. The boxes provide a tool for an instant analysis and
...
results in the morning and the afternoon, as
...
afternoon workouts are quite similar in average distance and duration, but the average pace of 3.2 min/km in the afternoon sessions seems quite considerably lower than for the morning sessions with average pace of 4.6 min/km. 
...
```{r eveningSessions, out.width='95%', fig.align='center', fig.cap='Afternoon workouts'}
...
processed data and the blue line is the data after
smoothing. 
...
the session spent in the zone. The plots summarize
and quantify training intensity distributions by the
time spent exercising in certain zones. The user is also able to evaluate their
...
In Figure \@ref(fig:zonesMultipleSessions), 21 sessions and the variables
speed and heart rate were selected, each split into 5 zones.
...
separately, the **trackeRapp** splits the users sessions by sport. The **trackeRapp** lets the user select which sports to display as shown in Figure \@ref(fig:sportClassification), where the user can select any of the sports and the user interface is automatically updated
...
\@ref(fig:changePointDetection) the maximum number of
changepoints is set to 3 changes for speed. However, the algorithm might detect fewer segments than the
...
```{r changePointDetection, out.width='95%', fig.align='center', fig.cap='Changepoint detection (dark red dash line'}
